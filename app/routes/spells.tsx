...
import { getSpells } from "~/models/card.server";
...
export type LoaderData = {
  cards: Awaited<ReturnType<typeof getSpells>>;
};

export const loader: LoaderFunction = async ({ request, params }) => {
  const url = new URL(request.url);
  const cards = await getSpells(toFilter(url.searchParams));
    ...
    <div className="flex h-full min-h-screen flex-col">
      <main className="flex h-full bg-white">
        <div className="h-full w-60 bg-lightbg">
          ...
          {FilterForm("SPELLS", submit, searchParams, data, Subtypes, Keywords)}
        </div>
