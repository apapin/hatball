...
import { getHeroes } from "~/models/card.server";
...
export type LoaderData = {
  cards: Awaited<ReturnType<typeof getHeroes>>;
};

export const loader: LoaderFunction = async ({ request, params }) => {
  const url = new URL(request.url);
  const cards = await getHeroes(toFilter(url.searchParams));
    ...
    <div className="flex h-full min-h-screen flex-col">
      <main className="flex h-full bg-white">
        <div className="h-full w-60 bg-lightbg">
          ...
          {FilterForm("HEROES", submit, searchParams, data, Subtypes, Keywords)}
        </div>
